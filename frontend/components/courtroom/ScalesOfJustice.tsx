  ...
  return (
    <div className="flex flex-col items-center w-full max-w-4xl mx-auto py-1">
      <div className="flex justify-between w-full text-[10px] font-mono font-bold uppercase tracking-widest mb-1.5 px-1">
        <span className={`transition-colors duration-700 ${score < 50 ? "text-[rgb(var(--heading))]" : "text-[rgb(var(--text-muted))]"}`}>
          Defense
        </span>
        <span className="text-[rgb(var(--text-muted))] font-normal text-[9px] tracking-widest">
          Balance
        </span>
        <span className={`transition-colors duration-700 ${score > 50 ? "text-red-600" : "text-[rgb(var(--text-muted))]"}`}>
          Prosecution
        </span>
      </div>

      <div className="relative w-full h-2.5 bg-[rgb(var(--border-sub))] rounded-full overflow-hidden">
        <div className="absolute top-0 bottom-0 left-1/2 w-px bg-[rgb(var(--text-fg))]/25 z-10" />
