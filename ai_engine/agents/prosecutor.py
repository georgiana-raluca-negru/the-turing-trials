...
def prosecutor_turn_node(state: MatchState) -> dict:
    ...
    available_evidence = [e.model_dump() for e in state["prosecution_evidence"] if not e.is_used]
    ...
    raw_response = chain.invoke({
        ...
        "history": history if history else "The trial has just begun. Make your opening statement.",
        "format_instructions": parser.get_format_instructions()
    })
    
    argument = clean_and_parse_json(raw_response.content, Argument)
    argument.speaker = "Prosecutor"
    
    used_ids = argument.attached_evidence_ids
    new_evidence_list = []
    for ev in state["prosecution_evidence"]:
